...
foreach ($bookingData as $delBooking) {
    if ($delBooking["check_out"] < date("Y-m-d")) {
        ...
        delete_where_fun("payment", "guest_id", $delId, $connection);
    }
}
}
?>
                                ...
                                <div class="col-3 pt-2 pr-2">
                                    ...
                                    foreach ($rate as $mainRate) {
                                        ...
                                        if ($startDate < $mainRate["start"]) {
                                            ...
                                            $seasonId = $mainRate["season"];
                                            $roomCount = 0;
                                            $availableRoomSql = "SELECT * FROM guests WHERE hotel_id = $hotel_id AND season = $seasonId";
                                    ...
                                    ?>
                                            ...
                                            <button class="btn <?php if($rooms > $availableRoomsbutton){echo "d-none";} ?> <?php if (count($price) == 0 || $mainRate["rooms"] <= $roomCount) {
                                            ...
                                            ?>
                                                <div class="alert alert-danger" role="alert">
                                                    We Cannot accomodate number of rooms required by you!
                                                    <?php echo "<br>". $availableRoomsbutton . " rooms are available!";?>
                                                </div>
                                            <?php
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    ?>
                                                        <div class="border bg-primary">
                                                            <div class="lato font-20 p-1"><span class="font-15 text-white"><?php echo $val ?></span> <a href="confirmation.php?stay=<?php echo $stayNights . "&hotel_id=" . $mainHotels["id"] . "&season=" . $seasonId . "&startDate=" . $startDate . "&endDate=" . $endDate . "&children=" . $children . "&adults=" . $adults . "&rooms=" . $rooms . "&price=" . $key . "&accomodation=" . $val . "&pickup=" . $pickup . "&room_no=" . $rateId[$rate_index]; ?>" class="p-2 btn btn-success mt-1 float-right ml-5">Book Now</a><span class="float-right mr-5 lato text-white font-weight-bold"><?php echo $key . "$" ?></span></div>
